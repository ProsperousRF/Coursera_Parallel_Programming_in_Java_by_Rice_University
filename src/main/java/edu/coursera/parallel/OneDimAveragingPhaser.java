...
/** Wrapper class for implementing one-dimensional iterative averaging using phasers. */
public final class OneDimAveragingPhaser {
  ...
  public static void runSequential(
      final int iterations, final double[] myNew, final double[] myVal, final int n) {
    double[] next = myNew;
    double[] curr = myVal;
  ...
  /**
   * An example parallel implementation of one-dimensional iterative averaging that uses phasers as
   * a simple barrier (arriveAndAwaitAdvance).
   *
   * @param iterations The number of iterations to run
   * @param myNew A double array that starts as the output array
   * @param myVal A double array that contains the initial input to the iterative averaging problem
   ...
   */
  public static void runParallelBarrier(
              ...
              () -> {
                ...
                for (int iter = 0; iter < iterations; iter++) {
                  ...
                  threadPrivateMyNew[left] =
                      (threadPrivateMyVal[left - 1] + threadPrivateMyVal[left + 1]) / 2.0;

                  final int right = (i + 1) * (n / tasks);
                  threadPrivateMyNew[right] =
                      (threadPrivateMyVal[right - 1] + threadPrivateMyVal[right + 1]) / 2.0;
